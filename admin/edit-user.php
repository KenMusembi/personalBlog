...
<div id="wrapper">
	...
	if(isset($_POST['submit'])){
		...
		if($role =='') {
			$error[] = 'Please indicae the role in lowercase';
		}
		...
		if(!isset($error)){
			...
			try {
				...
				if(isset($password)){
					...
					//update database
					$stmt = $db->prepare('UPDATE blog_members SET username = :username, email = :email, role = :role WHERE memberID = :memberID') ;
					$stmt->execute(array(
						':username' => $username,
						':email' => $email,
						':role' => $role,
	...
	<form action='' method='post'>
		...
		<input type='text' name='role' value='<?php echo $row['role'];?>'></p>
